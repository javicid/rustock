...
mod orchid_converter_tests {
    ...
    use crate::{empty_trie_hash, orchid_state_root, MemoryTrieStore, TrieKeySlice, TrieNode, TrieStore};
    use alloy_primitives::{Address, B256, U256};
    ...
    // ═══════════════════════════════════════════════════════════════
    // TrieConverter fixtures — ported from rskj TrieConverterTest.java
    // (deleted upstream commit a16069228). The expected roots were
    // generated by replaying the same operations on the real
    // ORCHID-0.6.1 codebase, so they validate the converter against the
    // actual orchid-era trie serialization that mainnet headers carry
    // between #729,000 and #1,591,000.
    // ═══════════════════════════════════════════════════════════════

    /// Faithful `java.util.Random` replica (48-bit LCG), needed to replay
    /// the rskj fixtures' op sequences byte-for-byte.
    struct JavaRandom {
        seed: u64,
    }
    ...
    impl JavaRandom {
        ...
        /// `java.util.Random#nextBytes`: 4 bytes per `next(32)`, LSB first.
        fn next_bytes(&mut self, len: usize) -> Vec<u8> {
            let mut out = vec![0u8; len];
            let mut i = 0;
            while i < len {
                let mut rnd = self.next(32);
                for _ in 0..usize::min(len - i, 4) {
                    out[i] = rnd as u8;
                    rnd >>= 8;
                    i += 1;
                }
    ...
    fn put_key(trie: TrieNode, key: &[u8], value: &[u8], store: &dyn TrieStore) -> TrieNode {
        trie.put(&TrieKeySlice::from_key(key), value, store)
    }

    /// Replays a TrieConverterTest fixture op sequence with
    /// MutableRepository-equivalent unitrie writes: 10 random accounts, the
    /// last 5 of which are contracts (setupContract marker + storage rows +
    /// code). `compressed_keys` selects the variant with 20-byte (left-padded)
    /// storage keys.
    fn build_converter_fixture(compressed_keys: bool, store: &dyn TrieStore) -> TrieNode {
        let mut rnd = JavaRandom::new(0);
        let mut trie = TrieNode::empty();
        for i in 0..10 {
            let addr = Address::from_slice(&rnd.next_bytes(20));
            // TestUtils.randomBigInteger(4): BigInteger(32 bits, rnd)
            let nonce = U256::from_be_slice(&rnd.next_bytes(4));
            // TestUtils.randomCoin(18, 1000)
            let balance = U256::from(10u64).pow(U256::from(18))
                * U256::from(rnd.next_int(1000) as u64);
            let account = AccountState::new(nonce, balance);
            trie = put_key(trie, &account_key(&addr), &account.encode(), store);
            if i >= 5 {
                // track.setupContract(addr)
                trie = put_key(trie, &storage_prefix_key(&addr), &[1], store);
                let (full_rows, padded_rows) = if compressed_keys { (2, 2) } else { (5, 0) };
                for _ in 0..full_rows {
                    let slot = B256::from_slice(&rnd.next_bytes(32));
                    let len = rnd.next_int(40) as usize + 1;
                    let value = rnd.next_bytes(len);
                    trie = put_key(trie, &storage_key(&addr, &slot), &value, store);
                }
                for _ in 0..padded_rows {
                    // DataWord.valueOf(randomBytes(20)): left-padded to 32
                    let mut slot = [0u8; 32];
                    slot[12..].copy_from_slice(&rnd.next_bytes(20));
                    let len = rnd.next_int(40) as usize + 1;
                    let value = rnd.next_bytes(len);
                    trie = put_key(
                        trie,
                        &storage_key(&addr, &B256::from(slot)),
                        &value,
                        store,
                    );
                }
                // randomCode(60): 4-byte 0xFF prefix + 1..=55 random bytes
                let code_len = rnd.next_int(55) as usize + 1;
                let code = [&[0xFF, 0xFF, 0xFF, 0xFF][..], &rnd.next_bytes(code_len)].concat();
                trie = put_key(trie, &code_key(&addr), &code, store);
            }
        }
        trie
    }
    ...
    /// rskj TrieConverterTest#test1Simple (ORCHID-0.6.1-generated root).
    #[test]
    fn orchid_converter_rskj_fixture_simple() {
        let store = MemoryTrieStore::new();
        let trie = build_converter_fixture(false, &store);
        let root = orchid_state_root(&trie, &store);
        assert_eq!(
            root,
            parse_hash("3513db97e8e9afff71727bce4b2ac01ff3b874e4814cbb31828829b730fd7eca")
        );
    }

    /// rskj TrieConverterTest#getOrchidAccountTrieRootWithCompressedStorageKeys
    /// (ORCHID-0.6.1-generated root; exercises left-padded storage keys).
    #[test]
    fn orchid_converter_rskj_fixture_leading_zero_storage_keys() {
        let store = MemoryTrieStore::new();
        let trie = build_converter_fixture(true, &store);
        let root = orchid_state_root(&trie, &store);
        assert_eq!(
            root,
