...
//! numbers whose most significant bit is set (sign extension). Standard alloy_rlp
//! rejects these as non-canonical. These helpers strip leading zeros before parsing.
...
mod tests {
    ...
    #[test]
    fn test_u8_with_leading_zero() {
        let encoded = [0x82, 0x00, 0x80];
        let mut buf = encoded.as_slice();
        let val = decode_u8_lenient(&mut buf).unwrap();
        assert_eq!(val, 0x80);
    }

    #[test]
    fn test_u8_overflow() {
        let encoded = [0x82, 0x01, 0x00];
        let mut buf = encoded.as_slice();
        assert!(decode_u8_lenient(&mut buf).is_err());
    }

    #[test]
    fn test_u32_with_leading_zero() {
        let encoded = [0x84, 0x00, 0x80, 0x00, 0x00];
        let mut buf = encoded.as_slice();
        let val = decode_u32_lenient(&mut buf).unwrap();
    ...
    #[test]
    fn test_u64_zero() {
        let encoded = [0x80]; // RLP for empty string = 0
        let mut buf = encoded.as_slice();
        let val = decode_u64_lenient(&mut buf).unwrap();
        assert_eq!(val, 0);
    }
