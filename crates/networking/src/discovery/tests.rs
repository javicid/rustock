...
async fn test_discovery_service_interaction() {
    ...
    let service1 = Arc::new(DiscoveryService::new("127.0.0.1:0", key1, table1.clone(), 33, local_node1).await.unwrap());
    ...
    let service2 = Arc::new(DiscoveryService::new("127.0.0.1:0", key2, table2.clone(), 33, local_node2).await.unwrap());
    let _addr2 = service2.socket.local_addr().unwrap();

    // 1. Service 2 pings Service 1
    service2.send_ping(addr1).await.unwrap();

    // 2. Service 1 should handle the ping and send a pong
    let mut buf = [0u8; 1280];
    let (n, addr) = service1.socket.recv_from(&mut buf).await.unwrap();
    service1.handle_packet(&buf[..n], addr).await.unwrap();

    // Verify service 1 added service 2 to its table
    {
        let table = table1.lock().await;
        assert!(table.get_all_nodes().iter().any(|n| n.id == id2));
    }

    // 3. Service 2 should receive a pong
    ...
    // 4. Service 2 sends FindNode to Service 1
    service2.send_find_node(id2, addr1).await.unwrap();
    let (n, addr) = service1.socket.recv_from(&mut buf).await.unwrap();
    service1.handle_packet(&buf[..n], addr).await.unwrap();

    // 5. Service 2 should receive Neighbors
    let (n, addr) = service2.socket.recv_from(&mut buf).await.unwrap();
    let packet = DiscoveryPacket::decode(&buf[..n]).unwrap();
    if let DiscoveryPayload::Neighbors(nks) = packet.payload {
        assert!(!nks.nodes.is_empty());
    } else {
