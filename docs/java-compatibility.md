...
This document catalogs behaviors that are consensus- or protocol-load-bearing
because of a **language or implementation artifact of `rskj`** — things a
spec-following client could implement "correctly" and still fork from the
network. Concretely: collection iteration order (Java `HashMap`/`HashSet`),
byte/endianness order, non-canonical or byte-exact serialization, integer
truncation/rounding, trie-existence vs EIP-161 emptiness, error paths that
still mutate state, gas/opcode behavior that contradicts the EIP it derives
from, uninitialized/default-state handling, and hash-preimage construction —
plus the foundational RLPx / serialization / sync-protocol framing (§§1–7).
...
segwit (reed800), Bridge event emissions, and hardfork opcode installs — are
logged in [`consensus-port-log.md`](./consensus-port-log.md). Those entries
describe behavior an RSKIP *specifies* (a from-scratch client reading the RSKIP
would match it). Some feature slices also surface a genuine implementation
quirk (a byte order, a truncation); where that happens the quirk is noted in
that doc next to its feature.
