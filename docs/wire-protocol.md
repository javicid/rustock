# RSK Wire Protocol & Sync Framing

The rskj-specific RLPx framing, message encoding, and sync protocol a client
must speak to talk to RSK peers, plus discovery liveness. Interop-load-bearing
but (except where noted) **not consensus-critical** — getting these wrong stalls
sync rather than forking state.
